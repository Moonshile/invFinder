...
let properties = [n_CacheStateProp; n_CacheStatePropHome; n_MemDataProp]


let protocol = {
  name = "n_flash_data_cub";
  types;
  vardefs;
  init;
  rules;
...
let () = run_with_cmdline (fun () ->
  ...
  let _mu_context = Murphi.set_context name (In_channel.read_all "n_flash_data_cub.m") in
  let _smv_context = Smv.set_context name (In_channel.read_all "flash_nodata.smv")
    ~escape:(fun inv_str ->
      let replace s d =
  ...
  let endF = eqn (
    var (record [global "Sta"; arr [("Proc", [paramfix "i2" "NODE" (intc 1)])]; global "CacheState"])
  ) (const _CACHE_E) in
  let startF = eqn (
    var (record [global "Sta"; arr [("Proc", [paramfix "i2" "NODE" (intc 1)])]; global "CacheState"])
  ) (const _CACHE_I) in
